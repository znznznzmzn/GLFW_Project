...
layout(location = 7) in vec4 frag_alpha;

uniform int  brush_type;
uniform vec3 brush_color;
uniform vec3 picking_pos;
uniform float range;
...
vec3 BrushColor(vec3 pos) {
    if (picking_pos.x == 0 && picking_pos.y == 0 && picking_pos.z == 0) return vec3(0, 0, 0);
    if (brush_type == 0) {
		if (length(pos.xz - picking_pos.xz) <= range) return brush_color;
    }
    else if (brush_type == 1) {
		vec2 direction = pos.xz - picking_pos.xz;
        if ((direction.x < range && direction.x > -range) &&
		    (direction.y < range && direction.y > -range)) return brush_color;
    }
    else if (brush_type == 2) {
        if (picking_pos.x - tan(PI / 6.0) * (picking_pos.z + range * 0.5 - pos.z) < pos.x &&
            picking_pos.x + tan(PI / 6.0) * (picking_pos.z + range * 0.5 - pos.z) > pos.x &&
            picking_pos.z - range * 0.5f < pos.z && picking_pos.z + range * 0.5 > pos.z) return brush_color;
    }
    return vec3(0, 0, 0);
}
...
void main() {
	Material material;
	material.diffuseColor = Mapping_Diffuse(frag_uv, frag_alpha);
	material.specularIntensity = Mapping_Specular(frag_uv, frag_alpha);
	material.normal = Mapping_Normal(frag_tangent, frag_binormal, frag_normal, frag_uv, frag_alpha);
	material.viewPos = frag_viewpos;    
	material.worldPos = frag_worldpos;
	
	outColor = 
		Lighting_Directional(material) + 
		Calc_Ambient (material) + 
		Calc_Emissive(material) + 
		vec4(BrushColor(frag_worldpos), 1.0);
}