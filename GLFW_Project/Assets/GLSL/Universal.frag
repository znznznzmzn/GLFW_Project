...
struct Material {
 ...
 // Mapping
vec4 Mapping_Diffuse(vec2 uv) {
	if (hasDiffuse) return texture(diffuseMap, uv);
...
void main() {
	Material material;
	material.diffuseColor = Mapping_Diffuse(frag_uv);
	material.specularIntensity = Mapping_Specular(frag_uv);
	material.normal = Mapping_Normal(frag_tangent, frag_binormal, frag_normal, frag_uv);
